...
def ExtractStats(county_estimate, target_date, num_prev_days):
    ...
    if not county_estimate:
        return (0, curr_infections)
...
def LoadMarshalInterconnect(marshal_files):
    interconnect_map = {}
    for marshal_file in marshal_files:
...
def LoadMonthlyMarshalInterconnect(marshal_files_by_month):
    raw_monthly_interconnect = {month : LoadMarshalInterconnect(marshal_files) for month, marshal_files in marshal_files_by_month.iteritems()}
